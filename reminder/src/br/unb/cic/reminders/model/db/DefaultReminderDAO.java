...
/**
 * A default implementation of the @see {@link ReminderDAO} 
 * interface.
 * 
 * @author positivo
 *
...
public class DefaultReminderDAO extends GenericDAO<Reminder> implements ReminderDAO {
	...
	public List<Reminder> listRemindersByCategory(Category category) throws DBException {
		try {
			db = dbHelper.getReadableDatabase();
			Cursor cursor = db.rawQuery(DBConstants.SELECT_REMINDERS_BY_CATEGORY, new String []{ category.getId().toString() });
	 ...
	 * 
	 * @see ReminderDAO#updateReminder(Reminder reminder)
	 * As it was asked on iceScrum, the update use the persist method 
	 * after removing the given reminder. If this was what was asked then
	 * it is done, otherwise the persist method can be upgraded to support
	 * the update functionality (if it isn't already implemented. I 
	 * guess it isn't) later by refactoring it.
	 */
	...
	/*
	 * Iterates over a cursor in order to build a list of 
	 * reminders.
	 */
