...
public abstract class ReminderActivity extends Activity {
	...
	protected Spinner spinnerDate, spinnerTime, spinnerPriority,
	spinnerCategory;
	...
	private void createReminder() {
		try {
			...
			Toast.makeText(getApplicationContext(),
					"Não foi encontrado um Google Calendar", Toast.LENGTH_SHORT)
					.show();
		} catch (ParseException e) {
			Toast.makeText(getApplicationContext(),
					"Transformação incorreta de data", Toast.LENGTH_SHORT)
					.show();
		} catch (Exception e) {
	...
	protected void updateDateFromString(String sDate) {
		...
		char sYear[] = { sDate.charAt(6), sDate.charAt(7), sDate.charAt(8),
				sDate.charAt(9) };
	...
	private Spinner getSpinnerCategory() throws Exception {
		...
		ArrayAdapter<Category> adapter = adapterCategoryGenerator
				.getSpinnerAdapter(categories, this);
	...
	protected Spinner getSpinnerCategory(List<Category> categories)
			throws Exception {
		Spinner spinner = (Spinner) findViewById(R.id.spinnerCategories);

		SpinnerAdapterGenerator<Category> adapterCategoryGenerator = new SpinnerAdapterGenerator<Category>();

		spinner.setAdapter(adapterCategoryGenerator.getSpinnerAdapter(
				categories, this));
	...
	private Spinner getSpinnerPriority() {
		...
		ArrayAdapter<Priority> priorityArrayAdapter = adapterPriorityGenerator
				.getSpinnerAdapter(priorityValues, this);
