...
public class ReminderAddActivity extends Activity {
	...
	private EditText edtReminder, edtDetails, edtDate, edtHour; //, edtPriority, edtCategory;
	...
	/*
	 * Just an auxiliary method to create reminders 
	 * from the UI data.
	 * 
	...
	private Reminder createReminder() {
		/*
		 * Since view's not passing all the information yet (priority
		 * ...), we instantiate a reminder initially passing only "text"
		 * and "date".
		 */
	...
	private Reminder getReminderFromIntent() {
		...
		if ("br.com.positivo.reminders.ADD_REMINDER".equals(action)
			...
			try {
				 ...
				 * 1. Return user to the calling activity.
	...
	private void initialize(Reminder reminder) {
		try {
			...
			//edtDate.addTextChangedListener(Mask.insert("##-##-####", edtDate));
			...
			//TODO: we really need to think about our exception handling approach
			e.printStackTrace();
		}
	}
