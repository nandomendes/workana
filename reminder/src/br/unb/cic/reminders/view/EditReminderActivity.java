...
public class EditReminderActivity extends ReminderActivity {
	...
	protected void initializeValues() {
		...
		spinnerPriority.setSelection(Priority.fromCode(
				Integer.parseInt(priority, 10)).getCode());
		try {
	...
	protected void persist(Reminder reminder) {
		...
		try {
			...
			if (category != null) {
				...
				Controller.instance(getApplicationContext()).addCategory(
		...
		try {
			Controller.instance(getApplicationContext()).updateReminder(
					reminder);
		} catch (DBException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	private int categoryToIndex(Category category) throws Exception {
		List<Category> categories = Controller
				.instance(getApplicationContext()).listCategories();
		int i = 0;
